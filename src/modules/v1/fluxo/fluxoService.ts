...
export class FluxoService implements IFluxoService {
    ...
    private getStatus(parada: SAU_PROGRAMACAO_PARADA): SAU_ITEM_LOOKUP {
    ...
    private async setStatusPp(parada: SAU_PROGRAMACAO_PARADA, status: string): Promise<SAU_PROGRAMACAO_PARADA> {
        switch(parada.ID_STATUS_PROGRAMACAO) {
            case 'P':
    ...
    public async nextLevel(parada: SAU_PROGRAMACAO_PARADA): Promise<SAU_PROGRAMACAO_PARADA> {
        ...
        case 'RASCUNHO':
            ...
            historico = this.sauHistProgramacaoParadaRepository.createDefaultHistorico(parada, 'AAPRV_USINA', parada.ID_STATUS_PROGRAMACAO);
        break;
        case 'AAPRV_USINA':
            await this.setStatusPp(parada, 'AAPRV_OPE')
            historico = this.sauHistProgramacaoParadaRepository.createDefaultHistorico(parada, 'AAPRV_OPE', parada.ID_STATUS_PROGRAMACAO);
        break;
        case 'AAPRV_OPE':
            await this.setStatusPp(parada, 'CONCL')
            historico = this.sauHistProgramacaoParadaRepository.createDefaultHistorico(parada, 'CONCL', parada.ID_STATUS_PROGRAMACAO);
        break;
        }
    ...
    public async prevLevel(parada: SAU_PROGRAMACAO_PARADA): Promise<SAU_PROGRAMACAO_PARADA> {
        ...
        switch(parada.idStatus.ID_ITEM_LOOKUP) {
          ...
          case 'AAPRV_OPE':
                ...
                historico = this.sauHistProgramacaoParadaRepository.createDefaultHistorico(parada, 'RASCUNHO', parada.ID_STATUS_PROGRAMACAO);
          break;
        }
